...
const Purchases = () => {
  ...
  const {products, brands, purchases ,firms} = useSelector(state => state.stock)
  const { getProPurBraFirms, deletePurchase } = useStockCalls();
  const {columns, handleSort, sortedData} = useSortColumn(purchases,columnObj)
 ...
 const filteredProducts = products?.filter((item)=> selectedBrands.includes(item.brand)).map((product)=> product?.name)

  const isBrandSelected = (purchase) =>{
    return selectedBrands.includes(purchase?.brand) || selectedBrands.length === 0;
  }

  const isProductSelected = (purchase) =>{
    return selectedProducts.includes(purchase?.product) || selectedProducts.length === 0;
  }
  ...
  return (
    <Grid container sx={{ flexDirection: "column" }}>
      <Grid item>
        ...
        <Button variant="contained" sx={{ margin: "1rem 0" }} onClick={()=>setOpen(true)}>
          New Purchase
        </Button>
      </Grid>
      ...
      >
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          ...
          <TableBody>
            {sortedData?.filter((purchase)=> isBrandSelected(purchase))?.filter((purchase)=> isProductSelected(purchase))?.map((item, index) => (
              <TableRow
                key={item?.id}
              ...
              >
                ...
                <TableCell align="center" sx={{ display: "flex" }}>
                  ...
                  <Box onClick={() => deletePurchase(item?.id)}>
